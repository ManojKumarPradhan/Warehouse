...
public class WhUserTypeExcelVew extends AbstractXlsxView {
	...
	private void setBody(Sheet sheet, List<WhUserType> whUserTypes) {
		...
		for (WhUserType whUserType : whUserTypes) {
			...
			row.createCell(3).setCellValue(whUserType.getForType());
			row.createCell(4).setCellValue(whUserType.getEmail());
			row.createCell(5).setCellValue(whUserType.getContact());
			row.createCell(6).setCellValue(whUserType.getIdType());
			row.createCell(7).setCellValue(whUserType.getIfOther());
			row.createCell(8).setCellValue(whUserType.getIdNum());
		}
	}
